...
namespace stan {
  namespace agrad {
    ...
    inline std::vector<chainable*>& var_stack_() {
      static thread_local std::vector<chainable*> var_stack;
      return var_stack;
    }

    inline std::vector<chainable*>& var_nochain_stack_() {
      static thread_local std::vector<chainable*> var_nochain_stack;
      return var_nochain_stack;
    }

    inline std::vector<chainable_alloc*>& var_alloc_stack_() {
      static thread_local std::vector<chainable_alloc*> var_alloc_stack;
      return var_alloc_stack;
    }

    inline memory::stack_alloc& memalloc_() {
      static thread_local memory::stack_alloc memalloc;
      return memalloc;
    }

    // nested positions
    inline std::vector<size_t>& nested_var_stack_sizes_() {
      static thread_local std::vector<size_t> nested_var_stack_sizes;
      return nested_var_stack_sizes;
    }

    inline std::vector<size_t>& nested_var_nochain_stack_sizes_() {
      static thread_local std::vector<size_t> nested_var_nochain_stack_sizes;
      return nested_var_nochain_stack_sizes;
    }

    inline std::vector<size_t>& nested_var_alloc_stack_starts_() {
      static thread_local std::vector<size_t> nested_var_alloc_stack_starts;
      return nested_var_alloc_stack_starts;
    }
